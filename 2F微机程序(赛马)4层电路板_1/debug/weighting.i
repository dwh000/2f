...
#line 9 "weighting.c" /0
 
 
 
 
 
 extern signed long xdata tare_weight;
 extern signed int xdata	zero_scale_value;
 extern unsigned long xdata filter2_out,load_cell_span;
 extern unsigned long xdata scale_output_val,scale_counterweight_val;
 extern unsigned char xdata filter_coefficient1;
...
#line 91 "weighting.c" /0
 ...
 ADC_CONV = 1;				 
  ...
  unsigned char get_adc(unsigned long *pADVal)
 {
  unsigned char i;
  unsigned int mmm = 0;
 ...
 EA = 1;
 *pADVal = ad_buf;
 if((ad_buf == 0)||(ad_buf == 0xffffff)) return 0XFE;
 return 0x2d;
 }
 ...
 {
  unsigned char  cnt;
  unsigned long adc_val,adc_min,adc_max,adc_sum,filter1_out;
 while(get_adc(&adc_val) != 0x2d) adc_init();
 adc_max = adc_val;
 adc_min = adc_val;
 adc_sum = adc_val;
 for(cnt = 0; cnt < (18 - 1);cnt++){
 while(get_adc(&adc_val) != 0x2d) adc_init();
 adc_sum += adc_val;
 ...
 return filter1_out;
  ...
  unsigned long filter1_clr(void) 
 {
  unsigned char  cnt;
  unsigned long adc_val,adc_min,adc_max,adc_sum,filter1_out;
 while(get_adc(&adc_val) != 0x2d) ; 
 adc_max = adc_val;
 adc_min = adc_val;
 adc_sum = adc_val;
 ...
 return result;
  ...
  signed long NW_measure_clr(void)				 
 {
 float result;
