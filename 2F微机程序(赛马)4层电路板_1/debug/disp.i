...
#line 10 "disp.c" /0
 ...
 sbit DIGIT1_ADDR = P7^0;
 sbit DIGIT2_ADDR = P7^1;
 sbit DIGIT3_ADDR = P7^2;
 sbit DIGIT4_ADDR = P7^3;
 sbit DIGIT5_ADDR = P7^4;
 sbit DIGIT6_ADDR = P7^5; 
 
 
  unsigned char code seg_sym[40] = {0xC0,0xF9,0xA4,0xB0,0x99,0x92,0x82,0xF8,0x80,0x90,
 0x88,0x83,0xC6,0xA7,0xA1,0x86,0x8E,0x89,0x8B,0xC7,
 0xAB,0xC8,0xA3,0x8C,0xAF,0x87,0xC1,0xBF,0xff,0xb9,
 0xf7,0XEF};
  unsigned char code seg_dot[40] = {0x40,0x79,0x24,0x30,0x19,0x12,0x02,0x78,0x00,0x10,
 ...
 if(sign_flag == 0){						 
 if(disp[4] == 0)	disp[4] = 28 ; 
 }
 else{
 if(sign_flag == 1)	disp[4] = 27; 
 else{
 disp_symbol(0X86);				 
 return;			
 }
 }
 disp_seg(seg_sym[disp[1]],0x01);			 
 disp_seg(seg_sym[disp[2]],0X02);			 
 disp_seg(seg_dot[disp[3]],0x04);			 
 disp_seg(seg_sym[disp[4]],0x08);			 
 return;
 }
 ...
 disp_seg(seg_sym[disp[0]],0x01);			 
 disp_seg(seg_sym[disp[1]],0X02);			 
 disp_seg(seg_sym[disp[2]],0x04);			 
 disp_seg(seg_dot[disp[3]],0x08);			 
 disp_seg(seg_sym[disp[4]],0X10);			 
 disp_seg(seg_sym[disp[5]],0x20);			 
  T0DelayMs(1);
 return;	
 }
 ...
 void disp_symbol(unsigned char disp_code)
 {
  unsigned char xdata disp[6] = {28,28,28,28,28,28};	
 switch(disp_code)
 {
 case 0x01:
 disp[3] = 22;
 ...
 case 0X8E:
 case 0X8F:
 disp[0] = (disp_code - 0X80)%10;	 
 disp[1] = (disp_code - 0X80)/10;	 
 disp[2] = 27;					
 disp[3] = 24;					
 disp[4] = 24;					
 disp[5] = 15;					
 ...
 case 0X2B:
 case 0X2C:	
 disp[1] = (disp_code - 0X20)%10;	 
 disp[2] = (disp_code - 0X20)/10;	 
 disp[3] = 30;					
 disp[4] = 30;					
 disp[5] = 16;					
 break;
 case 0X38:
 case 0X39:
 disp[1] = (disp_code - 0X37)%10;	 
 disp[2] = (disp_code - 0X37)/10;	 
 disp[3] = 30;					
 disp[4] = 23;					
 disp[5] = 14;
 break;
 ...
 void disp_int(unsigned int disp_data)
 {
  unsigned char xdata n,disp[6] = {28,28,28,28,28,28};
 for(n = 0;n<6;n++)						 
 {
 disp[n] = disp_data%10;
 disp_data /= 10;
 }
 if(disp[5] == 0)	disp[5] = 28;
 if((disp[4] == 0) && (disp[5] == 28))	disp[4] = 28;
 if((disp[3] == 0) && (disp[4] == 28))	disp[3] = 28;
 if((disp[2] == 0) && (disp[3] == 28))	disp[2] = 28;
 if((disp[1] == 0) && (disp[2] == 28))	disp[1] = 28;
